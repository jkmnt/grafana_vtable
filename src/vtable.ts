...
function create_field(field: DfField, options: VTableOptions, ctx: FieldCtx, is_dimension?: boolean): GridField {
  ...
  const common_unit = options.show_common_unit && field.type == FieldType.number ? discover_unit(field) : undefined;
...
function get_order(fields: DfField[], options: VTableOptions): number[] | undefined {
  ...
  if (field.type == FieldType.number) ordermap.sort((a, b) => num_comparer(a.v, b.v, sort.nullfirst, sort.desc));
  else ordermap.sort((a, b) => str_comparer(a.v, b.v, sort.nullfirst, sort.desc));
